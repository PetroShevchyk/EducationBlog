    ...
    <div class="mb-6 flex items-center justify-between">
                ...
                <template v-if="post.attachments.length">
                    <div v-if="format == 'image'">
                        <img v-for="image in post.attachments" v-bind:key="image.id" :src="file_url" class="w-full mb-4 rounded-xl">
                    </div>
                    <div v-if="format == 'video'">
                        <video v-for="image in post.attachments" v-bind:key="image.id"
    ...
    methods: {
        getImage() {
            axios
                ...
                .then(response => {
                    if (response.data.post.attachments.length){
                        if (['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg'].includes(response.data.post.attachments[0].get_image.split('.').pop().toLowerCase())) {
                            this.file_url = response.data.post.attachments[0].get_image
                            this.format = 'image'
                        } else if (['mp4', 'avi', 'mov', 'mkv'].includes(response.data.post.attachments[0].get_image.split('.').pop().toLowerCase())) {
                            this.file_url = response.data.post.attachments[0].get_image
                            this.format = 'video'
                        } else {
                            this.toastStore.showToast(5000, 'You used unsupportable type of file. Please try again', 'bg-red-300')
                        }
                    }
